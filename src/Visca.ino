...
#define VISCA_ERROR(code) \
  drawCommand(displayString + code); \
  return;
...
byte visca_pwr_inq_bytes[] = { 0x01, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x02, 0x81, 0x09, 0x04, 0x00, 0xFF };
  ...
  */
  ...
  if (waitForAck) {
    ...
    if (0x60 == (response[successByte] & 0xF0)) {
      ...
      VISCA_ERROR("EA" + String(stringBytes(&response[successByte + 1], 1)));
  ...
  if (waitForComplete) {
    ...
    if (0x60 == (response[successByte] & 0xF0)) {
      ...
      printBytes(response, 20);
      VISCA_ERROR("EC" + String(stringBytes(&response[successByte + 1], 1)));
    }
  }
  ...
  drawCommand(displayString + returnCode);
  // digitalWrite(PIN_TRANSMIT, LOW);
  //Serial.println("[done]");
}
...
boolean cameraStatus(int cameraNumber) {
  byte response[256];
  ...
  if (response[pwrByte] == 0x02) {
    ...
    return false; // Do something different? camera is there but on standy
  } else {
    return false; // camera didn't response
  }
}

void discoverCameras() {
  // BUG ??? just use IP with last octlet as 255?
  ...
  connect(cameraNumber);
  send(cameraNumber, &visca_pwr_inq_bytes[startPos], sendSize);
  while (recieve(cameraNumber, response)) {
